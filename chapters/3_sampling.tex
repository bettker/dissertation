...
\begin{algorithm}[H]
    ...
    \SetKwData{su}{s'}
    \SetKwData{t}{t}
    \SetKwData{Ms}{M\textsubscript{succ}}
    \SetKwData{Mh}{M\textsubscript{h}}
    ...
    \Fn{\SUI{\S}}{
        ...
        \Repeat{do not update any $h$-value}{
            \ForEach{$\s \in \Ms$}{
                \ForEach{$\t \in \Ms[\s]$}{
                    \If{$\Mh[\s] > \Mh[\t] + 1$}{
                        $\Mh[\s] \gets \Mh[\t] + 1$ \\
...
\centering
\begin{tikzpicture}
    \node[draw=black, fill=none, rounded corners, inner sep=5pt] (A) at (0.3,1.3) {Sampling by FSM};
...
\begin{figure}[h]
    \caption{An example of the approach applied in the Blocks World.}
    \label{fig:example}
    \addvspace{\baselineskip}
    \centering
    \begin{tikzpicture}
        ...
        % goal -> s1
        \draw[->] (2,0.2) to [bend left=10] (2.2,-1.5);
        ...
        % s1 -> s2
        \draw[->] (3.5,-3.6) to [bend right=20] (5.4,-3.7);
        ...
        % s1 -> s3
        \draw[->] (3.5,-1.5) to [bend left=20] (4.5,0);
        ...
        % s4 -> s5
        \draw[->] (13,-0.2) to [bend left=20] (12.5,-1.1);
